program example
    ...
    ShiftedSample_RK32  = genShifted(Sample_RK32    , -genMean(Sample_RK32  , dim = 2_IK), dim = 2_IK)
    ShiftedSample_RK64  = genShifted(Sample_RK64    , -genMean(Sample_RK64  , dim = 2_IK), dim = 2_IK)
    ShiftedSample_RK128 = genShifted(Sample_RK128   , -genMean(Sample_RK128 , dim = 2_IK), dim = 2_IK)
    ...
    ShiftedSample_RK32  = genShifted(Sample_RK32    , -genMean(Sample_RK32  , dim = 1_IK), dim = 1_IK)
    ShiftedSample_RK64  = genShifted(Sample_RK64    , -genMean(Sample_RK64  , dim = 1_IK), dim = 1_IK)
    ShiftedSample_RK128 = genShifted(Sample_RK128   , -genMean(Sample_RK128 , dim = 1_IK), dim = 1_IK)
    ...
    ! Shift the samples along the first dimension to the origin to have a mean of zero along the first dimension.
    ! In doing so, generate a transposed array. This can potentially save time compared to calling the `transpose()` intrinsic.

    ShiftedSample_RK32  = genShifted(Sample_RK32    , -genMean(Sample_RK32  , dim = 2_IK), dim = 2_IK, tenabled = .true.)
    ShiftedSample_RK64  = genShifted(Sample_RK64    , -genMean(Sample_RK64  , dim = 2_IK), dim = 2_IK, tenabled = .true.)
    ShiftedSample_RK128 = genShifted(Sample_RK128   , -genMean(Sample_RK128 , dim = 2_IK), dim = 2_IK, tenabled = .true.)

    ! Output the values to an external file.

    write(fileUnit,"(*(g0,:,' '))")
    write(fileUnit,"(*(g0,:,' '))") "Sample shape:"
    write(fileUnit,"(*(g0,:,' '))")
    write(fileUnit,"(*(g0,:,' '))") "original sample shape RK32:", shape(Sample_RK32)
    write(fileUnit,"(*(g0,:,' '))") " shifted sample shape RK32:", shape(ShiftedSample_RK32)
    write(fileUnit,"(*(g0,:,' '))")
    write(fileUnit,"(*(g0,:,' '))") "original sample shape RK64:", shape(Sample_RK64)
    write(fileUnit,"(*(g0,:,' '))") " shifted sample shape RK64:", shape(ShiftedSample_RK64)
    write(fileUnit,"(*(g0,:,' '))")
    write(fileUnit,"(*(g0,:,' '))") "original sample shape RK128:", shape(Sample_RK128)
    write(fileUnit,"(*(g0,:,' '))") " shifted sample shape RK128:", shape(ShiftedSample_RK128)
    write(fileUnit,"(*(g0,:,' '))")
    ...
    block
        ...
        ShiftedSample_1D = genShifted(Sample_1D, 1._RK)

        ! Output the values to an external file.

        write(fileUnit,"(*(g0,:,' '))")
        write(fileUnit,"(*(g0,:,' '))") "1D sample shifted by +1:"
        write(fileUnit,"(*(g0,:,' '))")
